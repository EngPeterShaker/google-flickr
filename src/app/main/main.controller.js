(function () {
  ...
  function MainController($timeout, webDevTec, $document, $resource, $scope, $rootScope, $window) {
    ...
    var FlickrAuth = {
      api_key: "e550f84233503cd16cb4815313c3f41d",
      format: "json",
      text: vm.searchText,
      nojsoncallback: 1,
      content_type: 4,
    ...
    function fetchSearch(text) {
      ...
      Search.get(FlickrAuth).$promise.then(function (result) {
        // success
        if (FlickrAuth.page > 1) {
          var tempArr = vm.items.photo;
    ...
    function showpopUp(index) {
      var PopRow = Number.isInteger(index / 6) ? Math.ceil(index + 1 / 6) : Math.ceil(index / 6);
      calcCol(index + 1);
      if (clickedIndex == index) {
        // if the same image clicked twice , close popup
        vm.items.photo.splice(6 * PopRow, 1);
        vm.popupShown = false;
      } else {
        ...
        vm.popupShown = true;
        clickedIndex = index;
        var clickedItem = vm.items.photo[index];
        var newItem = {
          farm: clickedItem.farm,
          id: "0",
          isfamily: 0,
          isfriend: 0,
          ispublic: 1,
          owner: clickedItem.owner,
          secret: clickedItem.secret,
          server: clickedItem.server,
          title: clickedItem.title,
          newID: clickedItem.id
        };

        vm.items.photo.splice(6 * PopRow, 0, newItem);
      }
    }
